...
enum Estado {
  ...
  EST_MENU, EST_MODO_B_INGRESO, EST_MODO_B_EJECUTANDO, EST_MODO_B_ERROR
};
...
void loop() {
  ...
  switch (estadoActual) {
    ...
    case EST_MODO_B_INGRESO:
      ...
      if (key) {
        if (key >= '0' && key <= '9') anguloStr[posAng] = key;
        else if (key == '*') { if (posAng < 2) posAng++; }
        else if (key == 'D') { strcpy(anguloStr, "000"); posAng = 0; }
        else if (key == '#') {
          ...
          if (anguloObj > 180) {
            ...
            actualizarPantalla("PRESIONE BTN1", "PARA INICIAR");
    ...
    case EST_MODO_B_ERROR:
      if (ticksTemp == 0) estadoActual = EST_MODO_B_INGRESO;
      break;
  }
}
